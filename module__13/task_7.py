...
def definition_depth(x):
    depth = x ** 3 - 3 * x ** 2 - 12 * x + 10
    return depth


x_min = 0.0
x_max = 4.0
safe = float(input("Введите максимально допустимый уровень опасности: "))
formula = x_min + (x_max - x_min) / 2
d_dop = abs(definition_depth(formula))

while d_dop > safe:
    if definition_depth(formula) > 0:
        x_min = formula
    else:
        x_max = formula
    formula = x_min + (x_max - x_min) / 2
...
print("Приблизительная глубина безопасной кладки:", formula, "м")
