...
for mb in range(speed, size_file, speed):
    print(
        f"Прошло {sec} сек. Скачано {mb} из {size_file} Мб ({ceil(100*mb/size_file)}%)"
    )
    sec += 1
else:
    print(
        f"Прошло {sec} сек. Скачано {size_file} из {size_file} Мб ({ceil(100 * size_file / size_file)})% "
    )
